      ...
      <main className="pt-32">
        <div className="mx-auto max-w-4xl px-6">
          ...
          <CaseStudyBlock
            ...
            body={p.gapsInExistingSolutions}
          />
          ...
          {p.architecture && p.architecture.length > 0 ? (
            ...
            <section className="mt-16">
              ...
              <div className="mt-4">
                <EmptyState
                  label={`Add architecture views for ${p.slug} in portfolio-data.ts → projects[].architecture`}
                />
              </div>
            </section>
          )}
          ...
          <section className="mt-16">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {p.stack.map((s) => (
                <span
                  key={s}
                  className="rounded-full border border-hairline px-3 py-1 font-mono text-xs text-foreground"
                >
                  {s}
                </span>
              ))}
          ...
          {(p.links?.github || p.links?.demo) && (
            <section className="mt-16 flex flex-wrap gap-3">
              {p.links.github && (
                <a
                  href={p.links.github}
                  className="rounded-full border border-hairline px-4 py-2 text-sm text-foreground hover:bg-accent"
                >
                  GitHub →
                </a>
              )}
              {p.links.demo && (
                <a
                  href={p.links.demo}
                  className="rounded-full bg-foreground px-4 py-2 text-sm text-background hover:bg-foreground/90"
                >
                  Live demo →
                </a>
              )}
